get_adjacency_matrix <- function(
    ...
    if (collapse_to_system_level) {
        ...
        return(list(G = G_hosp, B = B, hospitals = hospitals_in_target_area, systems = hospitals_to_include %>% select(sysname,miles,system_id) %>% 
                        unique()))
    } else {
        B <- G_hosp %>% get_incidence_matrix()
        return(list(G = G_hosp, B = B, hospitals = hospitals_in_target_area))
    }
